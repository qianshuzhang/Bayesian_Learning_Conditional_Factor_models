...
if sim == 1
    ...
    params.beta = [0.950;0.8;0.9];
    
    % 协方差矩阵分解A^{-1}
    % params.A = [1,0;0.2,1];
    params.A = [1,0,0;0.2,1,0;0.1,0.05,1];
    % params.A = [1,0,0;0,1,0;0,0,1];
    ...
    prior_mu.mu = 0.15/252*ones(1,n_factors);
    prior_mu.omega = 0.025/252*ones(1,n_factors);
    prior_mu.alpha = 0.03*ones(1,n_factors);
    prior_mu.beta = 0.8*ones(1,n_factors);
    ...
    prior_sig.mu = 0.1/252*ones(1,n_factors);
    prior_sig.omega = 0.01/252*ones(1,n_factors);
    prior_sig.alpha = 0.02*ones(1,n_factors);
    prior_sig.beta = 0.3*ones(1,n_factors);
...
while (t <= T)
    ...
    if Statistics.ESS(end) < 0.5*Nparam
        ...
        while or(counter == 0, size(unique(X(:,vary_param), 'rows'), 1) < 0.5*Nparam)
            ...
            [X, States, l, lnpdf_prior, Moves.AcceptRate(movecounter)] = ...
                    MoveSet(X, States, l, lnpdf_prior, prior_param, Ret(1:t,:),initial_h,n_factors,vary_param);
